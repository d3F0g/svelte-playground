...
// .svelte-kit/output/server/chunks/index-525aa5c6.js
function noop2() {
}
function run(fn) {
  return fn();
...
var init_layout_svelte = __esm({
  ".svelte-kit/output/server/entries/pages/__layout.svelte.js"() {
    ...
    _layout = create_ssr_component(($$result, $$props, $$bindings, slots) => {
      $$result.css.add(css);
      return `<div class="${"p-8 container mx-auto"}">${validate_component(Nav, "Nav").$$render($$result, {}, {}, {})}
...
var subscriber_queue2, pokemons, fetchPokemon, Pokecard, Routes;
var init_index_svelte = __esm({
  ".svelte-kit/output/server/entries/pages/index.svelte.js"() {
    ...
    fetchPokemon = async () => {
      ...
      const loadedPokemon = data.results.map((data2, index6) => {
        return {
          name: data2.name,
          id: index6 + 1,
          image: `https://raw.githubusercontent.com/PokeAPI/sprites/master/sprites/pokemon/${index6 + 1}.png`
        };
      });
      pokemons.set(loadedPokemon);
    };
    fetchPokemon();
    Pokecard = create_ssr_component(($$result, $$props, $$bindings, slots) => {
      let { pokemonDetails } = $$props;
      if ($$props.pokemonDetails === void 0 && $$bindings.pokemonDetails && pokemonDetails !== void 0)
        $$bindings.pokemonDetails(pokemonDetails);
      return `<a class="${"p-6 bg-gray-100 text-gray-800 text-center rounded-md shadow-sm hover:shadow-md flex flex-col items-center"}"${add_attribute("href", `/pokemon/${pokemonDetails.id}`, 0)}><img${add_attribute("src", pokemonDetails.image, 0)}${add_attribute("alt", pokemonDetails.name, 0)}>
    <h2 class="${"capitalize"}">${escape(pokemonDetails.id)}. ${escape(pokemonDetails.name)}</h2></a>`;
    });
    Routes = create_ssr_component(($$result, $$props, $$bindings, slots) => {
      let $pokemons, $$unsubscribe_pokemons;
      $$unsubscribe_pokemons = subscribe(pokemons, (value) => $pokemons = value);
      let searchTerm = "";
      let filteredPokemon = [];
      {
        {
      ...
      $$unsubscribe_pokemons();
      return `${$$result.head += `${$$result.title = `<title>Svelte Vite Pokedex</title>`, ""}`, ""}

<h1 class="${"text-4xl text-center my-8 uppercase"}">Svelte Vite Pokedex</h1>

<input type="${"text"}" placeholder="${"Search Pokedex"}" class="${"w-full rounded-md p-4 border-2"}"${add_attribute("value", searchTerm, 0)}>

<div class="${"grid gap-4 grid-cols-1 md:grid-cols-3 py-4"}">${each(filteredPokemon, (pokemon) => {
        return `
        ${validate_component(Pokecard, "Pokecard").$$render($$result, { pokemonDetails: pokemon }, {}, {})}`;
      })}</div>`;
    });
  }
});
...
var init_id_svelte = __esm({
  ".svelte-kit/output/server/entries/pages/pokemon/_id_.svelte.js"() {
    ...
    U5Bidu5D = create_ssr_component(($$result, $$props, $$bindings, slots) => {
      ...
      return `${$$result.head += `${$$result.title = `<title>${escape(pokemonData.name)}</title>`, ""}`, ""}

<div class="${"flex flex-col items-center"}"><h1 class="${"text-4xl text-center my-8 uppercase"}">${escape(pokemonData.name)}</h1>
    <p>Type: <strong>${escape(type)}</strong></p>
    <p>Height: <strong>${escape(pokemonData.height)}</strong></p>
    <p>Weight: <strong>${escape(pokemonData.weight)}</strong></p>
    <img class="${"my-4 h-40 w-40"}"${add_attribute("src", pokemonData.sprites["front_default"], 0)}${add_attribute("alt", pokemonData.name, 0)}></div>`;
    });
  }
});
